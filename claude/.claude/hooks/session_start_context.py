...
def main() -> int:
    ...
    context = (
        "This repository uses a split workspace layout. Treat `claude/` as the Claude "
        "working root, load instructions from `claude/CLAUDE.md`, and load extra settings "
        "from `claude/.claude/`. When reporting completion, include changed files and the "
