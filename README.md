...
| Claude | `claude/` | `claude/README.md`     | `claude/CLAUDE.md`, `claude/.claude/commands/`, `claude/.claude/rules/`, `claude/.claude/agents/` |
...
| `codex/.agents/skills/git-commit/` | staged changes 커밋 skill    |
...
**포함된 규칙과 워크플로우:** Cursor의 코드 품질, TypeScript, React, testing, accessibility, LLM 행동 가이드를 Claude용 rule 파일로 옮겨 `claude/.claude/rules/`에 포함합니다. Cursor command와 Claude에서 쓸 반복 workflow는 `claude/.claude/commands/`에 둡니다.
